...
class MarketData(BaseData):
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        self.all_df = self.load_all(root_dir, file_list=file_list, pattern=pattern)
        #self.all_df = self.all_df.sort_values(by=['Index'])  # datasets is presorted
        #self.all_df = self.all_df.set_index('Index')
        self.all_IDs = self.all_df.index.unique()  # all sample (session) IDs
        ...
        self.feature_names = ['Open','High','Low','Close','Volume','Dividends']
        self.all_df['Close'] = self.all_df['Close'].shift(-1)
        self.all_df.drop(index=self.all_df.index[-1],axis=0,inplace=True)
        #self.all_df = self.all_df.reset_index()
        self.labels_df = pd.DataFrame(self.all_df['Close'].copy())
        #self.labels_df.drop(index=self.labels_df.index[-1],axis=0,inplace=True)
        #self.labels_df = pd.DataFrame(self.labels_df.reset_index())
        self.feature_df = self.all_df[self.feature_names]
        self.all_IDs = self.all_df.index.unique()  # all sample (session) IDs


    def load_all(self, root_dir, file_list=None, pattern=None, labels=False):
        """
        Loads datasets from csv files contained in `root_dir` into a dataframe, optionally choosing from `pattern`
        Args:
            root_dir: directory containing all individual .csv files
        ...
        if self.n_proc > 1:
            ...
            logger.info("Loading {} datasets files using {} parallel processes ...".format(len(input_paths), _n_proc))
            with Pool(processes=_n_proc) as pool:
                all_df = pd.concat(pool.map(MarketData.load_single, input_paths))
        else:  # read 1 file at a time
            all_df = pd.DataFrame(MarketData.load_single(path) for path in input_paths) #needs column indexing, see threaded method above

        return all_df

    @staticmethod
    def load_single(filepath):
        df = MarketData.read_data(filepath)
        df = MarketData.select_columns(df)
        num_nan = df.isna().sum().sum()
        if num_nan > 0:
            logger.warning("{} nan values in {} will be replaced by 0".format(num_nan, filepath))
            df = df.fillna(0)
