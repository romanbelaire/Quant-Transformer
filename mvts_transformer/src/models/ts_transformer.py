...
def model_factory(config, data):
    ...
    if (task == "classification") or (task == "regression"):
        num_labels = len(data.class_names) if task == "classification" else 1  # dimensionality of labels
        if config['model'] == 'LINEAR':
    ...
    if task == "distribution":
        return TSTransformerEncoderDistregressor(feat_dim, max_seq_len, config['d_model'],
                                                    config['num_heads'],
                                                    config['num_layers'], config['dim_feedforward'],
                                                    num_classes=1,
                                                    dropout=config['dropout'], pos_encoding=config['pos_encoding'],
...
class TSTransformerEncoderDistregressor(nn.Module):
    """
    Distribution classifier/regressor. Can be either regressor or classifier because the output does not include
    softmax. Concatenates final layer embeddings and uses 0s to ignore padding embeddings in final output layer.
    """

    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, num_classes,
                 dropout=0.1, pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False):
        super(TSTransformerEncoderDistregressor, self).__init__()
        ...
        stdev_init = ch.zeros(num_classes)
    ...
    def calc_kl(self, p, q):
        ...
        assert shape_equal([-1, self.num_classes], p_mean, q_mean)
        assert shape_equal([self.num_classes], p_var, q_var)
